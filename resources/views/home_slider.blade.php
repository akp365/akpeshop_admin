...
<h4 class="page-section-heading">Home Slider</h4>
    @for($divCount=0; $divCount<3; $divCount++)
        @php 
            $rowIndex = $divCount+1; 

            $primaryImageIndex = "home_slider_image_" . $rowIndex . "_primary";
            $defaultImageIndex = "home_slider_image_" . $rowIndex . "_default";

            $primaryUrlIndex = "home_slider_url_" . $rowIndex . "_primary";
            $defaultUrlIndex = "home_slider_url_" . $rowIndex . "_default";

            $selectedCountryIndex = "home_slider_image_" . $rowIndex . "_selected_countries";

            //dd($rowIndex, $primaryImageIndex, $defaultImageIndex, $selectedCountryIndex, $primaryUrlIndex, $defaultUrlIndex);
        @endphp
        <div class="panel panel-default">
            <div class="panel-body">
                <form method="POST" id="topBannerForm" enctype="multipart/form-data" action="{{ route('save_home_slider') }}">
                    @csrf
                    ...
                    <div class="content" style="box-shadow: 0px 0px 5px 2px #dcdcdc; padding:5px;">
                        <h4 class="page-section-heading"><b>Image #{{$rowIndex}}</b></h4>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group form-control-default">
                                    ...
                                    <div id="{{ $primaryImageIndex }}" class="akp-photo-uploader" @if(isset($siteLook[$primaryImageIndex])) data-src="{{ env('AKP_STORAGE') . 'home_slider' . '/' .  $siteLook[$primaryImageIndex] }}" data-image-name="{{ $siteLook[$primaryImageIndex] }}" @endif ></div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group form-control-default">
                                    ...
                                    <!-- UPLOADER COMPONENT WILL BE INITIATED INSIDE THIS DIV -->
                                    <div id="{{ $defaultImageIndex }}" class="akp-photo-uploader" @if($siteLook[$defaultImageIndex] != "NA") data-src="{{ env('AKP_STORAGE') . 'home_slider' . '/' .  $siteLook[$defaultImageIndex] }}" data-image-name="{{ $siteLook[$defaultImageIndex] }}" @endif ></div>
                                </div>
                            </div>
                        </div>
