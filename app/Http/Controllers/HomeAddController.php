...
class HomeAddController extends Controller
{
    ...
    public function showHomeAdds(Request $request){
        ...
        return view('home_add', compact('siteLook', 'countryList'));
    }


    public function saveHomeAdd(Request $request){
        //dd($request->all());
 
        $this->rowIndex = $request->row_index;
        $this->primaryImageIndex = "home_add_image_" . $this->rowIndex . "_primary";
        $this->defaultImageIndex = "home_add_image_" . $this->rowIndex . "_default";
        $this->primaryUrlIndex = "home_add_url_" . $this->rowIndex . "_primary";
        $this->defaultUrlIndex = "home_add_url_" . $this->rowIndex . "_default";
        $this->selectedCountryIndex = "home_add_image_" . $this->rowIndex . "_selected_countries";
    ...
    private function processDefaultImage($selectedCountries, Request $request){
        if($selectedCountries != "99999") //PRIMARY WON'T SHOW 'WORLDWIDE', MEANS A DEFAULT BANNER IS NEEDED
        {
            if ($request->hasFile("photo_" . $this->defaultImageIndex)) { //IMAGE UPLOADED
                ...
                if ($request->file("photo_" . $this->defaultImageIndex)->isValid()) { //VALID UPLOAD
                    ...
                    if ($request->{"image_exists_" . $this->defaultImageIndex} == "yes") { //EXISTING DEFAULT BANNER FOUND
                        Storage::disk('akp_storage')->delete('home_add/' . $request->{"existing_image_name_" . $this->defaultImageIndex});//DELETE EXISTING BANNER BEFORE SAVING NEW ONE
                    }

                    $request->{"photo_" . $this->defaultImageIndex}->store('home_add', 'akp_storage'); //NOW SAVE NEW ONE
            ...
            Storage::disk('akp_storage')->delete('home_add/' . $request->{"existing_image_name_" . $this->defaultImageIndex});
        ...
        if ($request->hasFile("photo_" . $this->primaryImageIndex)) { //PRIMARY IMAGE IS UPLOADED
            if ($request->file("photo_" . $this->primaryImageIndex)->isValid()) { //VALID UPLOAD
                ...
                if ($request->{"image_exists_" . $this->primaryImageIndex} == "yes") { //DELETE PREVIOUSLY EXISTING PRIMARY BANNER
                    Storage::disk('akp_storage')->delete('home_add/' . $request->{"existing_image_name_" . $this->primaryImageIndex});
                }

                $request->{"photo_" . $this->primaryImageIndex}->store('home_add', 'akp_storage'); //SAVE NEW PRIMARY BANNER
