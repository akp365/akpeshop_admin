    ...
    public function order_more_info(Request $request)
    {
        if ($request->ajax()) {
            ...
            $ordersWithDetails = $orders->map(function ($order) {
                ...
                $order->currency_rate = $current_currency->usd_conversion_rate;
                $order->total_invoice_value = 0;
                $order->total_tax = 0;
                $order->total_price_with_vat = 0;
                $order->total_price_without_vat = 0;
                ...
                foreach ($orderDetails as $detail) {
                    ...
                    if ($detail->tax_type === "Included") {
                        $order->total_included_tax += $detail->tax * $detail->qty * $current_currency->usd_conversion_rate;
                    }



                    if ($detail->tax_type === "Excluded") {
                        $order->total_excluded_tax += $detail->tax * $detail->qty * $current_currency->usd_conversion_rate;
                    }
                    ...
                    $order->total_product_price += $detail->product_price;
                    $order->total_invoice_value += $detail->final_price;
                    $order->total_tax += number_format($detail->tax * $detail->qty * $current_currency->usd_conversion_rate, 2, '.', '');
                ...
                $order->final_invoice_value = number_format((($order->invoice_value) + $order->total_cod_fees + $order->seller_total_shipping_fee) - $order->total_discount, 2, '.', '');
